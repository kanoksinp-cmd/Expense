...
import sqlite3, os, datetime, logging, threading
...
def init_db():
    with db_transaction() as conn:
        conn.executescript('''
            ...
            CREATE INDEX IF NOT EXISTS idx_online_last_seen   ON online_status(last_seen);
        ''')
...
def add_member(trip_id):
    ...
    with db_transaction() as conn:
        conn.execute(
            'INSERT OR IGNORE INTO members(trip_id, name) VALUES(?,?)', (trip_id, name)
        )
...
@handle_errors
def vacuum_db():
    """Reclaim space and defragment the DB (run during off-peak hours)."""
    conn = get_conn()
    conn.execute('VACUUM')
